...
#   algorithm = eval(algo + "()")# set the algorithm...............................................amruta
    ...
    replace_index_value = predictions_w_item_info[predictions_w_item_info['year'] == 2024].index[0]
    print("Index of the first occurrence of the year 2024:", replace_index_value)
    ...
    max_prediction_index = svd_predictions['prediction'].idxmax()
    #max_pediction_row = svd_predictions[svd_predictions['prediction'] == svd_predictions['prediction'].max()]
    svd_predictions.loc[0], svd_predictions.loc[max_prediction_index] = svd_predictions.loc[max_prediction_index].copy(), svd_predictions.loc[0].copy()
    print(svd_predictions)
    subset = svd_predictions.iloc[0:2]
    print(subset)
    #svd_predictions = svd_predictions.sample(frac=1)
    A =pd.DataFrame(svd_predictions[2:5].sort_values('prediction', ascending=False))
    B =pd.DataFrame(svd_predictions[5:10].sort_values('prediction', ascending=False))
    svd_predictions_sorted = A._append(B)
    svd_predictions = subset._append(svd_predictions_sorted)
